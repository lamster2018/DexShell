package com.example.lahm.dexshell;
...
/**
 * Project Name:DexShell
 * Package Name:com.example.lahm.dexshell
 * Created by lahm on 2017/11/11 上午12:54 .
 * <p>
 * 代码参考
    ...
    protected void attachBaseContext(Context base) {
        ...
        try {
            ...
            RefInvoke.setFieldOjbect("android.app.LoadedApk",
                    ...
                    wr.get(),
                    dLoader);
            // 通过反射置换android.app.ActivityThread 中的mClassLoader为加载解密出APK的DexClassLoader
            // 该DexClassLoader一方面加载了源程序、另一方面以原mClassLoader为父节点
            // 这就保证了即加载了源程序又没有放弃原先加载的资源与系统代码。
...
//      用反射后的app去onCreate  super.onCreate();
        {
            loadResources(apkFileName);
            Log.i("demo", "onCreate-application");
            // 如果源应用配置有Application对象，则替换为源应用Application，以便不影响源程序逻辑。
            // 通过AndroidManifest.xml中的meta标签获取源程序Apk中的Application对象的。
            String appClassName = null;
            try {
                ApplicationInfo ai = this
                        .getPackageManager()
            ...
            ApplicationInfo appinfo_In_LoadedApk = (ApplicationInfo) RefInvoke.getFieldObject("android.app.LoadedApk",
                    loadedApkInfo,
                    "mApplicationInfo");
     ...
     * @throws IOException
     */
    private void splitPayLoadFromDex(byte[] apk_data) throws IOException {
        int ablen = apk_data.length;
        //取被加壳apk的长度   这里的长度取值，对应加壳时长度的赋值都可以做些简化
        byte[] dexlen = new byte[4];
        System.arraycopy(apk_data, ablen - 4, dexlen, 0, 4);
        ...
        //把被加壳apk内容拷贝到newdex中
        System.arraycopy(apk_data, ablen - 4 - readInt, newdex, 0, readInt);
    ...
    // //直接返回数据，要对应之前的加密方法
    private byte[] decrypt(byte[] src_data) {
